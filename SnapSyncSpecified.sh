...
#    ssh $remote "zfs list -H -t snap | grep $ZFS"
...
echo "end ccMD out"
    ...
    ssh $remote $ccMD >/dev/null || ( echo "Does not exist remotely." && echo $(tput setaf 4) && echo "Syncing snapshot $SNAPSHOT to $FULL_REMOTE" && echo $(tput sgr0) && echo $(tput setaf 154) && \
